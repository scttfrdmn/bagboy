...
func (p *Packager) Pack(ctx context.Context, cfg *config.Config) (string, error) {
	...
	// On macOS with hdiutil available, run the build script to produce a real DMG.
	if runtime.GOOS == "darwin" {
		if _, err := exec.LookPath("hdiutil"); err == nil {
			if err := p.runBuildScript(ctx, dmgDir, outputPath, cfg); err != nil {
				return "", fmt.Errorf("hdiutil build failed: %w", err)
			}
			return outputPath, nil
		}
	}

	// Non-macOS or hdiutil unavailable: write a scaffold file.
	mockDMG := fmt.Sprintf("# Mock DMG for %s %s\n# Generated by bagboy\n# On macOS, run: cd %s && ./build-dmg.sh\n", cfg.Name, cfg.Version, dmgDir)
...
func (p *Packager) runBuildScript(ctx context.Context, dmgDir, outputPath string, cfg *config.Config) error {
	cmd := exec.CommandContext(ctx, "bash", "build-dmg.sh")
	cmd.Dir = dmgDir
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("%w\n%s", err, out)
	}
	// The script creates the DMG inside dmgDir; move it to the expected output path.
	src := filepath.Join(dmgDir, fmt.Sprintf("%s-%s.dmg", cfg.Name, cfg.Version))
